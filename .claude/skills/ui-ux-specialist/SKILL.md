...
### Drag & Drop
- **dnd-kit**: ALWAYS use `@dnd-kit` for reordering.
...
### Loading & Layout Stability
- **Shell-First**: For data-heavy pages, ALWAYS move the layout shell (headers, action buttons, search/filters) OUTSIDE of the `isLoading` block. Only wrap the specific data-display component in the loading spinner so the user can see the page structure and access actions before the query finishes.
- **Loading UX**: ALWAYS use a loader (e.g., AntD `Spin`) for data fetching states. To prevent vertical layout shifts (CLS), wrap the loader in a container with a `minHeight` (e.g., `400px`) and `display: flex` to center the spinner. DO NOT use skeletons unless explicitly requested.
- **Layout Consistency**: For pages where a scrollbar might appear/disappear, avoid `mx: 'auto'` on centered containers — prefer fixed margins (e.g., `mx: 4`) with a `maxWidth`.
- **Full-Height Layouts**: The root layout should use flexbox with `minHeight: '100vh'` and a `flex: 1` content area to push the footer to the bottom.
...
- **Form Action Placement**: Place primary submit/action buttons outside of `Form.Item` at the bottom of the form, ensuring they remain inside the `<Form>` container.
- **Complete Creation Flows**: Ensure all related data (subtasks, tags, etc.) can be managed during the creation phase. Avoid 'empty' initial states that force the user to immediately edit a newly created item.

### Type Safety & Clean JSX
- **Options Separation**: ALWAYS move `Select`, `Radio.Group`, `Checkbox.Group`, and `Cascader` options out of the JSX. Use external constants for static options or component-level logic for dynamic ones.
- **Type-Safe Options**: Ensure `Select` options use the project's established unions (e.g., `TASK_STATUS_OPTIONS: { label: string; value: TaskStatus }[]`) to prevent runtime mismatches.

### Color Mapping for Domain Entities
When a domain entity (RDV type, priority, status…) needs a visual color, centralise the mapping in the **model file** alongside the Zod schema — never hardcode color strings inside components. Export a typed record and a helper function:

```typescript
// src/models/RdvModel.ts
export interface RdvTypeStyle {
  badge: string;  // subtle pill — bg-50, text-700, border-200
  block: string;  // prominent block — bg-200, hover:bg-300, text-800
}

export const RDV_TYPE_STYLES: Record<RdvType, RdvTypeStyle> = {
  'Urgence': { badge: 'bg-red-50 text-red-700 border-red-200', block: 'bg-red-200 hover:bg-red-300 text-red-800' },
  // …
};
...
- Use **two visual contexts**: `badge` (light pill in list cards) and `block` (solid background in time-grid or calendar cells). Both come from the same record — the component picks the right variant.
- **Warm-first palette**: For apps with warm amber/brown primaries, prefer warm-neutral shades for the default, muted cool-toned shades (violet-200, sky-200) for secondary types, and full red/amber for high-urgency types. Avoid saturated blues or greens that clash with the warm base.
- When adding the time-grid block variant, use `opacity-60` on secondary text (e.g., the time range) instead of a separate color token — this keeps the secondary text readable across all type colors without a per-type `textSecondary` field.
...
### Final Step
- Always run `reviewer-expert` after completing UI work.
