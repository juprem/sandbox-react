...
- **Atomic Operations**: Use transactions (`db.transaction`) for multi-step database updates **only when using the Neon WebSocket/serverless driver**. The `neon-http` driver does NOT support interactive transactions — use `db.batch()` for independent queries or sequential `await` calls for dependent ones instead.
