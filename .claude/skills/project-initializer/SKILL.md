...
- **tRPC**: Delivers end-to-end type safety for API calls, eliminating runtime mismatches between client and server.
...
### 5. tRPC Setup
- Wire the tRPC router into TanStack Start so all subsequent features can use end-to-end typed procedures.
- See [references/trpc-router-config.md](references/trpc-router-config.md) for the exact route configuration (`src/routes/api.trpc.$.ts`), the Axios-powered link setup, and the app-level provider wiring.
- This step is mandatory for any project that will use the `task-logic-expert` skill.
...
## References
See [references/trpc-router-config.md](references/trpc-router-config.md) for the exact set up of the tRPC route inside TanStack Router.
